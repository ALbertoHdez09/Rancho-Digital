...
import { ChevronLeft, Scale, Info, Syringe, CheckCircle2, Search, ScanLine } from 'lucide-react-native';
    ...
    if (!isConnected) {
      ...
      const ani = cache.find((a: any) => String(a.id) === String(id));
        if (ani) {
          ...
          Alert.alert("No encontrado", "Este animal no está en la memoria local.");
        }
...
async function buscarYVincularPadre() {
  ...
  try {
    ...
    if (!data) {
      Alert.alert(
        'No encontrado',
        'No tenemos registro de este arete en el corral. ¿Deseas registrarlo primero?',
        [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Registrar Nuevo', onPress: () => {
            setModalPadres({ ...modalPadres, visible: false });
...
async function borrarAnimal() {
  setModalOpcionesAnimal(false);
  Alert.alert('Eliminar Animal', `¿Seguro que quieres borrar al animal ${animal?.arete_siniiga}? Esta acción no se puede deshacer.`, [
    { text: 'Cancelar', style: 'cancel' },
    { text: 'Eliminar', style: 'destructive', onPress: async () => {
      try {
        if (!isConnected) {
          const cache = await obtenerCacheLocal('animales_cache') || [];
          await guardarCacheLocal('animales_cache', cache.filter((a: any) => String(a.id) !== String(id)));
          const queue = JSON.parse(await AsyncStorage.getItem('sync_queue') || '[]');
  ...
  async function guardarFecha() {
    try {
      const cambios = { fecha_nacimiento: fechaTemp || null };
      if (!isConnected) {
        const cache = await obtenerCacheLocal('animales_cache') || [];
        const idx = cache.findIndex((a: any) => String(a.id) === String(id));
        if (idx !== -1) { Object.assign(cache[idx], cambios); await guardarCacheLocal('animales_cache', cache); }
  ...
  return (
    <View style={styles.container}>
      ...
      {/* MODAL EDITAR FECHA */}
      <Modal animationType="slide" transparent visible={modalEditarFecha}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <View style={styles.modalOverlay}>
            <TouchableOpacity style={styles.modalDismissArea} onPress={() => setModalEditarFecha(false)} />
            <View style={styles.bottomSheet}>
              <View style={styles.sheetHandle} />
              <Text style={[styles.sheetTitle, { textAlign: 'left', width: '100%', marginBottom: 6 }]}>Fecha de Nacimiento</Text>
              <Text style={{ color: '#6B7280', fontWeight: '600', width: '100%', marginBottom: 20 }}>Formato: AAAA-MM-DD (ej: 2022-03-15)</Text>
              <View style={[styles.inputWrapperSearch, { borderColor: color, marginBottom: 20 }]}>
                <TextInput
                  style={{ flex: 1, fontSize: 18, color: '#111827', fontWeight: '700' }}
                  value={fechaTemp}
                  onChangeText={setFechaTemp}
              ...
              <TouchableOpacity style={[styles.saveModalBtn, { backgroundColor: color }]} onPress={guardarFecha}>
                <Text style={styles.saveModalBtnText}>Guardar Fecha</Text>
              </TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
