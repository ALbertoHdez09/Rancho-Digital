...
export default function NuevoPlanMedico() {
  ...
  async function guardarTodo() {
    if (!arete || !nombrePlan || dosis.some(d => !d.producto)) {
      Alert.alert('Faltan datos', 'Asegúrate de poner el arete, nombre del plan y nombre de todos los productos.');
      return;
    }
    ...
    try {
      ...
      if (isConnected) {
        ...
        if (!animalEnCache) {
          Alert.alert('Modo Offline', 'No encontramos este animal en la memoria local. Conéctate a internet para poder registrarle este plan.');
          setLoading(false);
          return;
        }
      ...
      Alert.alert('Error', _e.message || 'No se pudo guardar el plan.');
    } finally {
      setLoading(false);
    }
  }
