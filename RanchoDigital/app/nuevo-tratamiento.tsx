...
export default function NuevoTratamientoScreen() {
  ...
  async function guardarRegistro() {
    if (!animalSeleccionado || !producto || !dosis) {
      Alert.alert('Faltan Datos', 'Selecciona un animal, el producto y la dosis.');
      return;
    }
    ...
    try {
      ...
      if (isConnected) {
        ...
        try {
          ...
          Alert.alert('¡Guardado!', 'El registro médico se guardó correctamente en la nube. ☁️');
          router.back();
        } catch (error) {
  ...
  const registroParaCache = {
    ...
    planes_medicos: {
      ...
      animales: animales.find((a: any) => a.id === registro.animal_id) || {}
    }
  };
  await guardarCacheLocal('tareas_cache', [registroParaCache, ...cacheActual]);
  Alert.alert('Modo Offline', 'Tratamiento guardado en tu celular. Se sincronizará cuando regrese la red. 📡');
  router.back();
}
