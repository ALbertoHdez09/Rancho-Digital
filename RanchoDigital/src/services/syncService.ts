...
export async function agregarAlBuzon(tabla: string, operacion: 'INSERT' | 'UPDATE', datos: any) {
  try {
    const queueCachada = await AsyncStorage.getItem(SYNC_QUEUE_KEY);
    const queue = queueCachada ? JSON.parse(queueCachada) : [];
...
export async function sincronizarBuzon() {
  try {
    ...
    let queue = JSON.parse(queueCachada);
    ...
    for (const tarea of queue) {
      ...
      if (tabla === 'planes_medicos_compuesto' && operacion === 'INSERT') {
        ...
        if (errPlan) {
          ...
          const dosisAInsertar = dosis_relacionadas.map((d: any) => ({
            ...d,
      ...
      // 🆕 CASO 2: Plan médico con dosis embebidas (desde nuevo-registro-medico)
        ...
        if (errPlan) {
          ...
          const dosisAInsertar = dosis_medicas.map((d: any) => ({
            ...d,
            plan_id: planInsertado.id
          }));
          const { error: errDosis } = await supabase.from('dosis_medicas').insert(dosisAInsertar);
